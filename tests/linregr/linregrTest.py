...
output_test_sql_group = """
          ...
          select array_accum_madlib_temp_4545341231(array[r2] order by {g}) 
                  as r2 from {tbl_output};
          ...
          select array_accum_madlib_temp_4545341231(array[condition_no] order by {g}) 
                  as condition_no from {tbl_output};
          \\x off
          """ % (aggregate_sql)
...
# ------------------------------------------------------------------------
# ------------------------------------------------------------------------
class LinregrGroupOutputTestCase (MADlibTestCase):
    """
    Run templated SQL tests
    """
    # ----------------------------------------------------------------
    ...
    # Since all R result is in one file
    # We just need to read it once
    r_resultfile = "linregr_test_group.ans"
    Rresults = None
...
class LinregrGroupInputTestCase (MADlibTestCase):
  """
  Run input tests
  """
  # ----------------------------------------------------------------
  ...
  # use a different name convention from the above example
  template_method = "linregr_group_input_test_{incr_}"
